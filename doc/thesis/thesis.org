...
*** Working around compiler and toolchain bugs
    ...
    Therefore, in this section we unfortunately present a subset of the
...
***** Packet generation helper script utterly broken
      ...
      This script is completely broken: it does not even produce the right
      number of bytes.
      ...
      Due to the extremely low code quality, attempting to debug and fix the
      ...
      It would be better to not include buggy example scripts at all,
      especially when they serve as an excuse for not adding any
      documentation.
***** Changes to the code have highly unpredictable effects
      ...
      This makes it very difficult to track down problems, and causes constant
      anxiety for the programmer.
...
***** Silent failures, no way to detect errors early
      ...
      (thus costing the programmer extra time), or in some cases, it will not
      fail at all, use stale intermediate data instead of the erroneous parts,
      and do something unexpected, thus causing utter confusion and more wasted
      time.
      In order to preserve our own sanity, we ended up replacing all scripts
...
***** Control plane API issues
      ...
      This deviation from the standard is not documented anywhere and, frankly,
      makes no sense.
...
*** Other targets, higher throughput, other high-speed SCION applications
    ...
    256*200e6 = 51e9 TODO fix the numbers).
