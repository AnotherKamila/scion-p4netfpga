...
* Acknowledgements
  ...
  :UNNUMBERED: t
  :END:
  I would like to thank my advisors Prof. Dr. Adrian Perrig and Benjamin
  ...
  Finally, I wish to thank Andrés Hamann for bearing with me and for being an
  excellent rubber duck. Andrés is awesome.
...
***** The hard problem in FPGA design: Meeting timing
      ...
      speed of light is finite, and the gates need a bit of time to stabilise on
      the correct value.
      ...
      limits the maximum clock frequency), not the sum, and thus such trade-offs
      are a useful tool.
* Design overview
...
*** Working around compiler and toolchain bugs
    ...
    in the solutions), which constituted a large fraction of the problems,
...
***** Throughput
      ...
      Further, the small maximum queue sizes, which do not grow indefinitely,
      indicate that the processing is not the bottleneck and our design could
...
***** Packet loss (or lack thereof)
      The observed zero packet loss is a surprising result, and when we obtained
      it, we first thought we had made an experimental error.
...
***** Types of problems suitable for P4
      P4 is not a general-purpose language, but it does allow quite general
      programs for packet processing.
...
******* P4 tables
        ...
        (Note that we have not tested TCAM, i.e. ~lpm~ tables. We assume that
