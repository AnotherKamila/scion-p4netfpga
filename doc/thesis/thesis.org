...
******* No way to cleanly add a custom extern
       ED: [2019-08-30 Fri 17:28]
       NetFPGA scripts expect information about all externs to live in one
       on file distributed with the NetFPGA repository.
        assumption is present in several other NetFPGA scripts.
       , adding a project-specific extern, such as our AES implementation,
        difficult problem.
       pent significant effort trying to do it cleanly, but we found no
        options.
       ing the issue with the NetFPGA developers got vague agreement, but
       roposal for a concrete patch did not receive a reply.
       , building this project still requires modifying a Python file that
       s with the P4-NetFPGA distribution.
...
*** TODO Area and timing constraints
    ...
    Originally, the critical path in the full design took over 6 nanoseconds.
    Running on a 200MHz clock requires all paths to take no longer than 5.0 ns
    (200 MHz ⇒ 5 ns per cycle).
    After more than a month of optimisation, we have been able to optimise that
    by a full 1 ns (i.e. 20%).
    However, we have not been able to cross the threshold: the longest path in
    the full design still requires 5.08ns.
    ...
    did not solve it.
    Thus, we have not been able to test our full design on real hardware;
    however, we have tested it in simulation, and we have tested two designs
    with a subset of features enabled, as discussed in the analysis in section
    [[*Analysis]].
...
* [3/3] Evaluation
  ...
  TODO does this paragraph make me sound lame?
*** DONE Method
...
***** Packet loss (or lack thereof)
      ...
      Thus, neither the input nor the output queues get full, and we are able to
      achieve zero packet loss.
*** TODO Guidelines for high-speed packet processing
    see [[*Introduction to designing for FPGAs]] for general intro to FPGAs
...
***** DONE What SCION does right
      ...
      consumption.
      Thus, exchanging a large CAM table for a relatively simple AES core is a
