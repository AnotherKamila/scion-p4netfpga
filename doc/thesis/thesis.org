...
*** DONE Contributions of this project
    ...
    We target a NetFPGA SUME board, which (in the ideal case) allow us
    ...
    evaluating the feasibility of building "SCION on chip", as much of the
...
***** DONE Overview of the process
      ...
      1. Logic description: writing the source code of your project.
      ...
      2. Describing the full design: taking the various modules (including the one/ones
      ...
      4. Implementation: placing and routing the gate-level logic onto the FPGA.
         ...
         circuits.
      5. Generating the bitfile and flashing it to the FPGA: Creating a binary
...
***** NEXT Control plane
      ...
      viewpoint.
      Therefore, this section presents a brief overview of
...
*** TODO Packet loss
***** Large packets (1500B):
      | Load |  time | Avg queue size | Max queue size | Tx frames | Rx frames | Loss |
      |------+-------+----------------+----------------+-----------+-----------+------|
      |  20% | 16:18 |                |                |  39094558 |  39094558 |    0 |
      |  40% | 16:22 |                |                |  79240424 |  79240424 |    0 |
      |  60% | 16:23 |                |                | 122369587 | 122369587 |    0 |
      |  80% | 16:16 |                |                | 157131302 | 157131302 |    0 |
      | 100% | 16:25 |                |                | 198083742 | 198083742 |    0 |
      #+TBLFM: $7=$6 - $5
...
115B packets
      | Load |  time | Avg queue size | Max queue size |  Tx frames |  Rx frames | Loss |
      |------+-------+----------------+----------------+------------+------------+------|
      |  20% | 17:04 |                |                |  334798373 |  334798373 |    0 |
      |  40% | 17:05 |                |                |  674862021 |  674862021 |    0 |
      |  60% | 17:07 |                |                |  990142312 |  990142312 |    0 |
      |  80% | 17:08 |                |                | 2125564720 | 2125564720 |    0 |
      | 100% | 17:01 |                |                | 1631918616 | 1631918616 |    0 |
      #+TBLFM: $7=$6 - $5
******* raw data:
********* 20%: 17:04
...
* [4/6] Future work
  TODO: 3 points are enough, otherwise your work might look "negligible"
...
*** INPR Higher throughput
    ...
    TODO The previous paragraph should actually go somewhere into [[*Analysis]]
...
[fn:11] The current version of SCION supports a "continue" flag in the hop field, which indicates that the hop field is longer than 8 bytes. There is no limit on the HF size. Because this causes significant issues in hardware implementations, the next version of SCION will only allow fixed-size hop fields.

[fn:10] https://github.com/NetFPGA/P4-NetFPGA-public/wiki/Workflow-Overview#p4-netfpga-extern-library

[fn:9] Unfortunately, we do not know the original source: we got this implementation from another student without any information despite asking.
