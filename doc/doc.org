...
*** TODO [2/3] week 3 first half: Figure out the parser despite setbacks
...
*** TODO [0/3] Week 3 second half: Make it run
***** WAIT Test the toolchain: Complete the =switch_calc= exercise
      The `master` branch of the NetFPGA repo is broken, but `vivado-2018.2`
      works. Synthesis finished, just waiting for physical access so I can
      reboot the host.
***** TODO Add an encaps deparser
***** TODO [0/4] Create the project structure needed for testing it
