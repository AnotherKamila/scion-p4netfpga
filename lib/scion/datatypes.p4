...
struct error_data_t {
    ERROR   error_flag; // in parsers, if verify() is supported, it is used instead of this
    bit<64> debug;      // used as a debug signal => no fixed meaning, can change without notice
}
